...
sed -i "s/{domain}/$ESCAPED_DOMAIN/g" "$PROJECT_ROOT/nginx/sites/$SAFE_FILENAME.conf"
sed -i "s/{port}/$PORT/g" "$PROJECT_ROOT/nginx/sites/$SAFE_FILENAME.conf"

# Создаем директорию для SSL
mkdir -p "$PROJECT_ROOT/nginx/ssl/sites/$DOMAIN"

# Генерируем тестовый SSL сертификат
openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
    -keyout "$PROJECT_ROOT/nginx/ssl/sites/$DOMAIN/privkey.pem" \
    -out "$PROJECT_ROOT/nginx/ssl/sites/$DOMAIN/fullchain.pem" \
...
echo "📝 Конфиг: $PROJECT_ROOT/nginx/sites/$SAFE_FILENAME.conf"
echo "🔐 SSL: $PROJECT_ROOT/nginx/ssl/sites/$DOMAIN/"
