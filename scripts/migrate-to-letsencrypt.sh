...
if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Использование: $0 <domain-file> <email>"
    echo "Пример: $0 easyfund_aldar_space algenzalo@gmail.com"
    exit 1
fi
...
if [ ! -f "$PROJECT_ROOT/nginx/sites/$DOMAIN_FILE.conf" ]; then
    echo "❌ Конфиг $PROJECT_ROOT/nginx/sites/$DOMAIN_FILE.conf не найден"
    ls "$PROJECT_ROOT/nginx/sites/"*.conf | xargs -n 1 basename
    exit 1
fi
...
sed -i \
  ...
  "$PROJECT_ROOT/nginx/sites/$DOMAIN_FILE.conf"

echo "5. Перезагружаем nginx..."
docker compose exec nginx nginx -s reload

echo "✅ Миграция $FULL_DOMAIN завершена!"
echo "📁 Сертификаты: $PROJECT_ROOT/certbot/conf/live/$FULL_DOMAIN/"
